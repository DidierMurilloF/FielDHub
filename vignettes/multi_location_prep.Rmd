...
Partially replicated (p-rep) designs are frequently used in early-generation plant breeding field trials. 
This design typically involves repeating some entries/breeding lines, while the remaining entries only 
appear once in the experiment. In `FielDHub`, the optimized p-rep design is a variant of the p-rep design 
that employs the principles of incomplete block designs (IBD) to determine the distribution of replicated 
and non-replicated treatments across multiple locations. 

## Optimization

The optimization problem that we try to solve is the following: Given a matrix $X$ of integers (p-rep design within location), we want to ensure that the distance between any two occurrences of the same treatment is at least a minimum distance $d$. More specifically, we want to modify $X$ to ensure that no treatments appears twice within a distance less than $d$ in the resulting matrix.

The goal of the optimization process is to find a modified matrix that satisfies this constraint while minimizing some measure of deviation from the original matrix $X$. In this case, the measure of deviation is the pairwise Euclidean distance between occurrences of the same treatment. The process is done by the function `swap_pairs()` that uses a heuristic algorithm that starts with a minimum distance of $d = 3$ between pairs of occurrences of the same treatment, and increases this minimum distance by $1$ and repeats the process until either the algorithm no longer converges or the maximum number of iterations is reached.
...
Consider a p-rep design where ten treatments are twice and 40 are once. The matrix (field layout) for this experiment has 6 rows and 10 columns.
...
If we calculate the pairwise distance between the rep treatments 
...
For the optimization designs we can see the new distances for each pairwise of treatments.


```{r}
print(B$distances[[length(B$distances)]])
```

As we can see the minimum distance that the algorithm reached is $5.0$. This means that no treatments appears twice within a distance less than $5.0$ in the resulting prep design.
...
environments but in different amounts of replications. We can generate this design using the 
FielDHub Shiny app and the FielDHub `multi_location_prep()` standalone function in R.
