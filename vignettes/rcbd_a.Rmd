...
vignette: >
  %\VignetteIndexEntry{augmented_RCBD}
...
The augmented randomized complete block design is another option to try to overcome the problem of limited facilities or lack of seed when plant breeders want to test a large number of varieties. Same amount of exemplar varieties (checks) are included as controls in every augmented block to try to provide an estimate of experimental error.

In this design you can set the number of entries/treatments, number of checks and the number of augmented blocks for the experiment. You can also choose to run the same experiment over multiple locations. 


## Generate an Augmented RCBD in FielDHub app

In this tutorial, we will show how to use the FielDHub App and the function `RCBD_augmented()` to generate an augmented randomized complete blocks design. For example, we say a project needs to test 60 genotypes of wheat over two locations with four checks and six augmented blocks. This design setup comes out with 6 blocks of size 24 plots for a total of 144 plots that will be distributed in a field of 12 rows and 12 columns.
...
1. Enter the number of stacked experiments in the **Input # of Stacked Expts** box. This means the number of times the experiment will be replicated. In our case we will perform just one experiment.
...
8. To ensure that randomizations are consistent across sessions, we can set a seed number in the box labeled **Seed Number**. In this example, we will set it to 1987. 
...
*   `lines = 405` is the number of entries
*   `checks = 5` is the number of checks in each augmented block.
...
*   `exptName = "Wheat_Aug_2022"` is an optional name for experiment.
...
*   `seed = 1987` is the seed number to replicate identical randomizations.
...
p1 <- plot(aug_RCBD, l = 1)
p1$layout
```

As you can see we can pass the specific location to plot the layout. Let us see the plot for the location 2.

#### Plot layout for location 2

```{r, fig.align='center', fig.width=7.2, fig.height=5.5}
p2 <- plot(aug_RCBD, l = 2)
p2$layout
```
