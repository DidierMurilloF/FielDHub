...
Then, we set the number of treatments per replication with the **Input # of Treatments** box. The number of treatments must be a composite number in the row-column design. The number of rows per replication is set with the **Input # of Rows** box and the number of replications of the whole plots is set with the **Input # of Full Reps** box. 
...
You can run the same design with a function in the FielDHub package, `row_column()`. We can enter the information describing the above design like this:
```{r, echo = FALSE, warning=FALSE, comment='', include=FALSE}
library(knitr)
library(FielDHub)
rcd <- row_column(t = 45,nrows = 5,r = 3,l = 1,seed = 123)
```
```{r, results='hide'}
rcd <- row_column(t = 45,     #number of treatments
...
print(rcd)
```
This returns a list consisting of all the information displayed in the output tabs in the FielDHub app: design information, plot layout, plot numbering, entries list, and field book. These are accessible by the `$` operator, i.e. `rcd$layoutRandom` or `rcd$fieldBook`. 
