...
vignette: >
  %\VignetteIndexEntry{alpha_lattice}
...
Then, we input the number of treatments in the **Input # of Treatments** box. The number of treatments must be a composite number in the alpha lattice design. The number of replications of these treatments is selected with the **Input # of Full Reps**. The next box, **Input # of Plots per IBlock**, sets the number of plots in each incomplete block.
...
# Function : `alpha_lattice()`

You can run the same design with a function in the FielDHub package, `alpha_lattice()`. We can enter the information describing the above design like this:
```{r, echo = FALSE, warning=FALSE, comment='', include=FALSE}
library(knitr)
library(FielDHub)
alpha <- alpha_lattice(t = 55,
                       k = 5,
                       r = 3,
                       l = 1,
                       seed = 123)
...
```
```{r, results='hide'}
alpha <- alpha_lattice(t = 55,     # number of treatments
                       k = 5,      # number of plots per incomplete block
...
print(alpha)
```
This returns a list consisting of all the information displayed in the output tabs in the FielDHub app: design information, plot layout, plot numbering, entries list, and field book. These are accessible by the `$` operator, i.e. `alpha$layoutRandom` or `alpha$fieldBook`. 
