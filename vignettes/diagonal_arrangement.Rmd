...
We will show how to generate a Single Diagonal Arrangement in `FielDHub` app step by step through an example. We say a project needs to test 300 genotypes of barley in a field containing 15 rows and 22 columns of plots. In this example, these 300 genotypes are allocated into one experiment. In addition, five checks are included in a systematic diagonal arrangement across field to fill 30 plots representing 9.09% of the total number of experimental plots.
...
1. **Import entries' list?** Choose whether to import a list with entry numbers and names for genotypes or treatments.
    ...
    * If the selection is `Yes`, the entries list has to fulfill a specific format and it must be a `.csv` file. The entries list file must have the columns `ENTRY` and `NAME`. The `ENTRY` column is for the unique entry integer number that identifies the treatments/genotypes. The column `NAME` is for the name of the treatments/genotypes. Both ENTRY and NAME must be unique, duplicated are not allowed. As Follow we show an example of data input for the entries list. In this example, we have an entry list with four checks and 8 treatments/genotypes. It is important to allocate the checks in the top part of the file. 
...
3. Select 6 from the drop down on the **Input # of Checks** box. 
...
9. Enter the name of the site/location in the **Input the Location** box. In this experiment we want the site is `FARGO`.
...
For printing a summary of the information that is in the object `single_diag`
...
`single_diag$fieldBook` is a list containing information about every plot in the field, with information about the location of the plot and the treatment in each plot. As seen in the output below, the field book has columns for `ID`, `EXPT`, `LOCATION`, `YEAR`, `PLOT`, `ROW`, `COLUMN`, `CHECKS`, `ENTRY`, and `TREATMENT`.

```{r, comment='', echo=FALSE}
print(head(single_diag$fieldBook), 10)
```

### Plot field layout

For ploting the layout that lay onb the cpordanes `ROW` and `COLUMN` in the fielbook object we can use the generic function `plot()` as follow,
...
12. Enter the name of the site/location in the **Input the Location** box. In this experiment we set the site as `FARGO`.
...
A variation on the single diagonal arrangement included in the `diagonal_arrangement()` function is the multiple diagonal arrangement, where the experiment is split into blocks of specified size. An example of a multiple diagonal arrangement is as follows:
