...
Then, we set the number of rows per replication with the **Input # of Horizontal Strips** box and the number of columns per replication with the **Input # of Vertical Strips** box. The number of replications of the whole plots is set with the **Input # of Full Reps** box. 
...
library(knitr)
library(FielDHub)
strip <- strip_plot(Hplots = 6,Vplots = 4,b = 3,l = 1,seed = 123)
```
```{r, results='hide'}
strip <- strip_plot(Hplots = 6, #number of horizontal strips
                    Vplots = 4, #number of vertical strips
                    b = 3,      #number of replications (blocks)
...
print(strip)
```
This returns a list consisting of all the information displayed in the output tabs in the FielDHub app: design information, plot layout, plot numbering, entries list, and field book. These are accessible by the `$` operator, i.e. `strip$layoutRandom` or `strip$fieldBook`. 
