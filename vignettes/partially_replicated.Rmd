...
reasons. In this design, it is recommended to replicate at least 1/3 of the treatments.

In this design you can set the number of entries that will have reps, as well as the number of 
...
We would like to acknowledge Mr. Jean-Marc Montpetit  for contributing code and ideas 
for the `swap_pairs()` and `pairs_distance()` functions. His contributions have had a 
...
two times each, and 130 entries only appearing once. This field trial is arranged in 
...
well as leave with just one copy the genotypes that are siblings (130).
