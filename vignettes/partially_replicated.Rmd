...
Partially replicated designs are commonly employed in early generation field trials. This type of design is characterized by replication of a portion of the entries, with the remaining entries only appearing once in the experiment. 
...
## 1. Generate a Partially Replicated Design in FielDHub App

As an example, considered a field trial with 300 plots containing 85 entries appearing two times each, and 130 entries only appearing once. This field trials is arranged in a field of 15 rows by 20 columns.
...
## 2. Generate a prep design by the `partially_replicated()` function
...
*   `locationNames = "PALMIRA"` is the name for the location
