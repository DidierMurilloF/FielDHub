...
## Inputs
First, select "No" in the **Import entries' list** box, as we are entering our own information about the experiment. We can choose whether to use the split-split-plot design in a RCBD or CRD with the **Select SSPD Type** box.
Then, we set the number of whole plots in the design with the **Whole-plots** box. In each whole plot we can set the number of sub plots contained with the **Sub-plots Within Whole-plots** box. The split-split-plot design adds an additional level of sub sub plot, the number of which we can set in the **Sub-Sub-plots within Sub-plots** box. The number of replications of the whole plots is set with the **Input # of Full Reps** box. 
...
You can run the same design with a function in the FielDHub package, `split_split_plot()`. We can enter the information describing the above design like this:
```{r, echo = FALSE, warning=FALSE, comment='', include=FALSE}
library(knitr)
library(FielDHub)
sspd <- split_split_plot(wp = 3,sp = 2,ssp = 4,reps = 3,type = 2,l = 1,seed = 123)
```
```{r, results='hide'}
sspd <- split_split_plot(wp = 3,     #number of whole plots
                         sp = 2,     #number of sub plots per whole plot
                         ssp = 4,    #number of sub sub plots per sub plot
...
print(sspd)
```
This returns a list consisting of all the information displayed in the output tabs in the FielDHub app: design information, plot layout, plot numbering, entries list, and field book. These are accessible by the `$` operator, i.e. `sspd$layoutRandom` or `sspd$fieldBook`. 
