...
- Fixed a bug in
  ...
  `seq(starting_dist, minDist, 1)` errored with “wrong sign in ‘by’
  argument”; the distance range is now guarded and the input design is
  returned unchanged when no swap distance is feasible.

- Fixed a bug in
  [`CRD()`](https://didiermurillof.github.io/FielDHub/reference/CRD.md)
  where a single experimental unit (`t = 1`, `reps = 1`) made the plot
  numbering `sample(plotNumber:(plotNumber + N - 1))` collapse to a full
...
- Fixed a bug in
  ...
  where a single character treatment matched no input branch (the third
  `else if` duplicated the second), leaving the treatment count `nt`
  unset and the call failing with “object ‘nt’ not found”; the branch
  now triggers the “requires more than one treatment” validation
  message.
