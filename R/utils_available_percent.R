...
available_percent <- function(n_rows, n_cols, checks, Option_NCD = FALSE, Visual_ch = NULL, visualCheck = FALSE,
  ...
  # print("we are here")
  # print(w_map_list)
  # w_map_list <- w_map_list[!sapply(w_map_list, is.null)]
  ...
  if(multi == FALSE && visualCheck == TRUE){
    ...
    colnames(M) <- c("Options", "Percentage of Checks", "Total # of Check Plots", "Visual Checks",
    ...
    colnames(M) <- c("Options", "Percentage of Checks", "Total # of Check Plots", "Total # of Fillers",
                     "Total # of Experimental Plots", "Total # of Plots")
    ...
    if(Option_NCD == TRUE){
      ...
      colnames(M) <- c("Options", "Percentage of Checks", "Total # of Check Plots", "Total # of Fillers",
                       "Total # of Experimental Plots", "Total #r of Plots")
      ...
      colnames(M) <- c("Options", "% of Diagonal Checks", "Total # of Check Plots",
                       "Total # of Experimental Plots", "Total # of Plots")
  ...
  for (m in opts) {
    ...
    visual <- visualCheck
    vs <- data.frame(index = 1:4,option = c("1-Top Left", "1-Top Left & 1-Top Right",
                                            "1-Top Left & 2-Top Right","1-Top Left & 3-Top Right"))
    ...
    if (kindExpt == "RDC" || kindExpt == "SUDC") {
      ...
      if (visualCheck == TRUE){
        ...
        if (visual_index == 1){
          ...
          w_map[1,1] <- Visual_ch
          w_map[1,ncol(w_map)] <- Visual_ch
          
        }else if (visual_index == 3){
          w_map[1,1] <- Visual_ch
          w_map[c(1,2),ncol(w_map)] <- Visual_ch
          
        }else if (visual_index == 4){
          w_map[1,1] <- Visual_ch
          w_map[c(1,2,3),ncol(w_map)] <- Visual_ch
          
        }
        
        n_Checks <- length(which(w_map == 1))
        vis <- length(which(w_map == Visual_ch))
        pots <- nrow(w_map) * ncol(w_map)
        per <- round((n_Checks/pots)*100,1)
        expt_lines <- pots - n_Checks - vis
        M[m, c(1,2,3,4,5,6)] <- c(m, per, n_Checks, vis, expt_lines, pots)
        ...
        # print(c(dim_expt, real_dim_data_entry, n_Checks))
        # print(Fillers)
        # print(Fillers > (n_cols - 5) || Fillers < 0)
        if(Fillers > (n_cols - 5) || Fillers < 0) next
          ...
          if (n_rows %% 2 == 0){
            if(planter_mov1 == "serpentine") {
              ...
              repeat{
                ...
                if (sum(w_map[1, ] == "Filler") == Fillers) break else i <- i + 1
              }
              # w_map[w_map == "-9"] <- "Filler"
            }else{
              i <- n_cols
            ...
            repeat{
              ...
              # print("I am stuck")
              # if (identical(c(n_rows, n_cols), c(15, 57))) print(w_map)
              # if (i == 0) break
      ...
      if (visualCheck == TRUE) {
        ...
        if(myWay == "By Column") {
          ...
          n_Checks <- length(which(w_map == 1))
          vis <- sum(w_map == Visual_ch)
          pots <- nrow(w_map) * ncol(w_map)
          per <- round((n_Checks/pots)*100,2)
          expt_lines <- pots - n_Checks - vis
          
          #f_expt_lines <- expt_lines - vis
        ...
        if (myWay == "By Row"){
          ...
          auto_cuts <- automatically_cuts(data = w_map, planter_mov = planter_mov1,
                                          way = "By Row", dim_data = data_dim_each_block)
          ...
          split_map_checks <- turner::matrix_to_blocks(w_map, blocks = rev(my_row_sets), byrow = TRUE)
          ...
          for(l in which_blocks_Fillers){
            if (!is.matrix(split_map_checks[[l]])) {
              split_map_checks[[l]] <- matrix(split_map_checks[[l]], ncol = length(split_map_checks[[l]]),
                                              byrow = T)
            } 
          ...
          Fillers_info <- matrix(data = c(which_blocks, Fillers_app), ncol = 2, byrow = FALSE)
          ...
          if(all(Fillers_info[2:nrow(Fillers_info),1] == 0) && Fillers_info[1,1] != 0){
            ...
            rows_each_block <- c(length(nrow(w_map):new_sort_cuts[1]), nrow(w_map) - length(nrow(w_map):new_sort_cuts[1]))
                ...
                if (rows_each_block[j] %% 2 == 0){
                  ...
                  if(skip_checks){
                    ...
                    repeat{
                      ...
                      if (sum(w_map[l, ] == "Filler") == len_Filler) break else k <- k + 1
                  ...
                  if(skip_checks){
                    ...
                    repeat{
                      ...
                      w_map[w_map[,] == "-9"] <- "Filler"
                      if (sum(w_map[l, ] == "Filler") == len_Filler) break else i <- i + 1
                    }
                    # w_map[w_map[,] == "-9"] <- "Filler"
                  }else{
