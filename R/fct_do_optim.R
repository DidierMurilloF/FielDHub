...
multi_location_prep <- function(
    ...
    if (missing(nrows) || missing(ncols)) {
        ...
        nrows <- numeric(l)
        ncols <- numeric(l)
        for (site in seq_len(l)) {
            options <- prep_dimension_options(
                total_plots = experimental_plots[site],
            ...
            nrows[site] <- options$rows[1]
            ncols[site] <- options$columns[1]
        }
    }
    ...
    output <- list(
        ...
        fillerField = design_randomization$fillerField,
